...
- **Public call-signs (anonymization §9):** STRAT-001 → **TAI**, STRAT-002 → **SIMBA**. Internal id
  (STRAT-001/002) + pair = SIRI ya server. Lessee anaona call-sign tu — hajui pair/logic/id.
...
| **Lessee** | Call-sign(s) zake tu (TAI/SIMBA), scorecard kwa lugha rahisi, matokeo + sheria | IP (logic/features/params), pair, internal id, models za wengine | Uaminifu + kulinda IP + privacy (token §9) |
...
|---|---|---|---|
| A | **STATUS BAND** | Call-sign, version, hadhi (PAPER/LIVE), **status light** + sentensi: *"TAI inatoa faida kama ilivyoahidi. Iko salama."* | steward.json + registry |
...
**Lessee:** A–G zote LAKINI (1) call-sign (TAI/SIMBA), si STRAT-xxx; (2) pair/params/features/logic
...
1. **AWAMU 1 — MODEL SCORECARD (INTERNAL):** sehemu A–G kwa PD role, kwa data tuliyonayo
   (steward.json + paper_log). Call-sign registry (TAI/SIMBA↔STRAT). Ndio moyo — thibitisha ndani.
